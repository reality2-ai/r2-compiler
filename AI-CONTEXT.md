...
**r2-compiler** is a visual composer for Reality2 (R2) firmware. The operator picks a carrier board (e.g. DFR1117 ESP32-C6), a set of R2 plugins, and a set of R2 sentants from a catalogue arrayed around a canvas, and the tool drives a Claude Code session that produces a flashable per-carrier firmware crate.

The compile work is delegated to Claude Code (`claude -p '<brief>' --output-format=stream-json`) running on the operator's workstation — the **same way Roy already works with r2-workshop manually**, but driven by the visual composition instead of free-form chat. r2-compiler does NOT contain a from-scratch code generator; it composes briefs and orchestrates CC sessions.
...
The operator adds new entries by dialog with the agent (you, when invoked through r2-compiler's compile-console pane). Every entry MUST be a self-contained directory — code + datasheets + AI-CONTEXT — that a future fresh CC session can pick up cold.
...
4. [`specifications/SPEC-R2-COMPILER.md`](specifications/SPEC-R2-COMPILER.md) — what r2-compiler IS, in canonical R2 vocabulary (class string, hive role, plugin shape).
...
**Scaffolding phase.** The directory tree, top-level docs (AGENTS.md, AI-CONTEXT.md, PROCESS.md, README.md), and the two normative specs (SPEC-R2-COMPILER, SPEC-CATALOGUE-LAYOUT) exist. No Rust has been written yet. No catalogue entries are populated yet — the three target boards (`esp32-s3-devkitc`, `esp32-s3-xiao`, `esp32-c6-dfr1117`) have placeholder dirs only.
...
- Don't invent a "r2-compiler-specific" plugin schema. `plugin.toml` per R2-PLUGIN §12.3 is the only source of truth.
