...
5. On `status == 0x00` the push is **confirmed**: the device returns `OK` only *after* it has streamed the firmware through SHA-256 and matched it byte-for-byte against the preamble hash, so the ack cryptographically proves the bytes that landed in the inactive OTA slot. The orchestrator records `firmware_sha` = the pushed SHA on the roster row (state stays `reachable`) and appends a `flashed_ota` history entry. The device then `esp_restart()`s ~2 s later and the connection drops.
6. **No post-reboot confirmation is awaited.** The earlier draft had the orchestrator wait 90 s for a beacon carrying the new `firmware_sha` — but R2-BEACON (the Legacy 28-byte AD) carries **no firmware identity**, and r2-workshop wire-v1 devices emit no `r2.update.applied` event, so there is nothing observable to wait for. The OTA ack (step 5) is the confirmation. `firmware_ver` is left unset — no semantic version exists anywhere in the v0.1 build pipeline (the artefact is a bare `firmware.bin`); populating it is a future concern once the compose step emits version metadata.
7. **Rollback is not currently observable.** A device that boots the new firmware, fails its R2-UPDATE §5 health check (boot integrity, ≥1 radio up, identity-intact = can sign a TG event, core-protocol = can encode/decode an R2-WIRE msg) within its tier window (§5.3: Tier-1 60 s / Tier-2 120 s / Tier-3–4 300 s), and rolls back to the previous slot gives the orchestrator no wire-v1 signal today. The roster's `firmware_sha` therefore reflects *what was successfully written and verified*, not a proven *running* version. Closing this gap requires the R2-UPDATE event model — the device emitting the TG-scoped R2-WIRE events `r2.update.applied {package_id, seq, payload_type, result: "ok"|"rollback", previous_seq}` (§3.1.4) on success and `r2.update.rollback {package_id, seq, reason, reverted_to_seq}` (§5.4/§7.1) on health-check failure; authoritative success = `applied(result="ok")` **and** no `rollback`. Confirming the *new version* is live is a re-query (`#ota_query`→`#ota_info`, CBOR key 2 = firmware version, §3.1.2; or `r2.update.query`→`r2.update.query_response`, §7.1), **not** a beacon read. Correlation back to the enrolled device uses `package_id` (the CSPRNG-unique id assigned per package, §2.2) plus the event's TG-signing identity (DEV_PK / hive_id) — **never** the rotating `rbid`. Neither R2-BEACON nor the current r2-workshop firmware provides any of this; tracked as a forward item (see §12).

> **Confirmation-model correction (2026-06-06).** Steps 5–7 originally specified a 90 s wait for a beacon carrying the new firmware's `firmware_sha`. That is not implementable: R2-BEACON has no firmware field, and R2-UPDATE confirms via TG-scoped `r2.update.applied`/`rollback`/`rejected` events (§3.1.4/§7.1), not beacons — which current r2-workshop devices don't emit either. Per a 2026-06-06 ruling (Roy), v0.1 adopts the **ack-as-confirmation** model above: the device-verified OTA ack is the `firmware_sha` confirmation; no post-reboot wait. The R2-UPDATE event model is the forward path for true running-version / rollback confirmation. Implemented in **F5** (`orchestrator/src/substrate/ota_push.rs`, the push + ack) and **F5b** (`orchestrator/src/sentants/roster.rs`, `firmware_sha` persistence on `deploy.device.done`).

> **Wire correction (2026-06-06).** §6.2's request/response framing originally described a text-line shape (`[u32 BE length]` frame, `OK <sha256>` / `ERR <reason>` / `REBOOTING` lines). That was an incorrect assumption — confirmed against R2-UPDATE §3.1.2.2 and r2-workshop's device source `crates/r2-esp/src/ota_tcp.rs` (which agree byte-for-byte): the real wire is binary-framed, the size is **little-endian**, the SHA-256 travels as **32 raw bytes** (not hex) in the request preamble, and EOF is signalled by a half-close, not a length prefix.
...
| R2-UPDATE event model — consume `r2.update.applied{result:"ok"\|"rollback"}` (§3.1.4) + `r2.update.rollback` (§5.4/§7.1), gated on the device's §5 health check (tier window 60/120/300 s), to confirm a *running* version + detect rollback (the ack-as-confirmation model in §6.2 only proves what was written, not what booted). Confirm the new version via re-query (`#ota_query`→`#ota_info` CBOR key 2 / `r2.update.query`). Correlate by `package_id` (§2.2) + TG-signing identity (DEV_PK/hive_id), **never** the rotating `rbid`. Requires r2-workshop firmware to emit these events. | Phase 2 |
| `firmware_ver` population — needs the compose step to emit semantic version metadata alongside `firmware.bin` (today the artefact is a bare image, so only `firmware_sha` is recordable); or read it back via the §3.1.2 `#ota_info` re-query | Phase 2 |
| Enrolled→`reachable`/`unreachable` state machine driven by beacon liveness. Correlation must NOT key on a stored `rbid` (it rotates per R2-BEACON §6.1 privacy HMAC, so a remembered value fails across reboots): compute the device's expected rbid for the current epoch from its session-key-derived schedule (the R2-DISCOVERY §3 resolver, per R2-TRANSPORT §2.1.3) and match, or correlate via signed events. Provisioning currently sidesteps this with unique-pending-slot disambiguation. | Phase 2 |
