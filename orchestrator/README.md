...
- Listens for `r2.compiler.*` events from the browser hive at `/r2` (WebSocket).
- Runs the plugins (compiler subprocess management, cargo, esptool, webfetch, git, …) that do the actual work.
- Holds the TG KeyHolder material (off-tree at `~/.config/r2-compiler/apiaries/<name>/tg_signer/`).
...
- Single binary `r2-compiler-orchestrator` built from `orchestrator/src/main.rs`.
...
| Compiler plugin (central `r2.compiler.build.*` driver) | Phase 1.7 |
...
|---|---|---|
| Server | `python3 -m http.server` | `r2-compiler-orchestrator` (axum) |
